...
async def send_weather_3_days(msg: Message, state: FSMContext) -> object:
    ...
    в days 3 дня
    на полторы секунды отпраляется сообщение с просьбой подождать
...
async def send_weather_tomorrow(msg: Message, state: FSMContext) -> object:
    ...
    если город указан неверное, то устанавливается стейт incorrect_city_for_tomorrow,
    отправляется сообщение с ошибкой и кнопкой Попробовать снова
    """
...
async def send_weather_today(msg: Message, state: FSMContext) -> object:
    """
    стейт current_day, сообщение от пользователя после выбора периода и отправки названия города.
    устанавливается активность бота - печатает...
    вызывается функция get_current_weather, в city передается название города, который написал пользователь,
    на полторы секунды отпраляется сообщение с просьбой подождать

    если город указан верно, то устанавливается стейт weather_sent и
    отправляется сообщение с прогнозом погоды на сегодня и кнопкой В меню

    если город указан неверное, то устанавливается стейт incorrect_city,
    отправляется сообщение с ошибкой и кнопкой Попробовать снова
    """
...
async def back_to_menu(callback: CallbackQuery, state: FSMContext) -> object:
    """
    стейт weather_sent, после отправки сообщения с прогнозом погоды пользователь нажал кнопку В меню
    устанавливается стейт menu и отправляется главное меню
    """
    await state.set_state(Periods.menu)
...
async def try_again(callback: CallbackQuery, state: FSMContext) -> object:
    """
    принимет все стейты, после ввода неверного города и нажатия кнопки Поробовать снова

    если стейт incorrect_city, то вызывается фукнция с вводом города для прогноза на Сегодня
    если стейт incorrect_city_for_tomorrow, то вызывается фукнция с вводом города для прогноза на Завтра
    если стейт incorrect_city_for_3_days, то вызывается фукнция с вводом города для прогноза на На три дня
    """
